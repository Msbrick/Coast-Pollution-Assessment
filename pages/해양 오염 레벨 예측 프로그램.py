...
if "Pollution Level" not in df.columns:
    st.error("'Pollution Level' 컬럼이 존재하지 않습니다. CSV 내용을 확인하세요.")
    st.stop()
...
X = df.drop(columns=["Pollution Level"])
...
st.write(f"🌡️ 예측된 오염도 레벨: **{predicted:.2f}**")
